...
func (s *Server) handleStatisticsFull(c *gin.Context) {
	...
	// history endpoint (handler_order.go). One-click "FXOS Autopilot" relaunches
	...
	if strings.EqualFold(strings.TrimSpace(trader.GetName()), "FXOS Autopilot") && strings.TrimSpace(userID) != "" {
