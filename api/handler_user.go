...
// (`fxos reset-password` / `fxos reset-account`, see cli.go), which requires
...
func (s *Server) createDefaultStrategies(userID string, lang string) error {
	...
	locales := map[string]strategyLocale{
		"zh": {
			defaultStrategy: strategyI18n{"FXOS Claw402 Auto Strategy", "The only built-in strategy: read the Claw402.ai board each cycle, fetch Signal Lab and cost/liquidation heatmap per candidate, then decide with raw candles."},
		},
		"en": {
			defaultStrategy: strategyI18n{"FXOS Claw402 Auto Strategy", "The only built-in strategy: read the Claw402.ai board each cycle, fetch Signal Lab and cost/liquidation heatmap per candidate, then decide with raw candles."},
		},
		"id": {
			defaultStrategy: strategyI18n{"Strategi Otomatis FXOS Claw402", "Satu strategi bawaan: membaca papan Claw402.ai, mengambil Signal Lab dan heatmap biaya/likuidasi per kandidat, lalu memutuskan dengan candle mentah."},
		},
	}
