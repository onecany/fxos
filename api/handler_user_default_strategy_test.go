...
func TestCreateDefaultStrategiesUsesOneReadyToRunClaw402Preset(t *testing.T) {
	st, err := store.New(t.TempDir() + "/fxos.db")
	if err != nil {
	...
	defaultStrategy := byName["FXOS Claw402 Auto Strategy"]
	if defaultStrategy == nil || !defaultStrategy.IsActive {
		t.Fatalf("FXOS Claw402 Auto Strategy should exist and be active")
	}
...
func TestCreateDefaultStrategiesMigratesLegacyPresetsWithoutOverridingActiveCustom(t *testing.T) {
	...
	if byName["FXOS Claw402 Auto Strategy"] != 1 {
		t.Fatalf("expected exactly one FXOS Claw402 Auto Strategy, got names=%+v", byName)
	}
