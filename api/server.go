...
func (s *Server) setupRoutes() {
	...
	api := s.router.Group("/api")
	{
		...
		s.route(api, "GET", "/hyperliquid/connect-config", "Get FXOS Hyperliquid builder authorization config", s.handleHyperliquidConnectConfig)
		...
		// is now a local CLI run on the host — `fxos reset-password` /
		// `fxos reset-account` — which requires shell access the attacker lacks.
		// See cli.go.
...
StrategyConfig fields:
  ...
  indicators.fxosos_api_key: ALWAYS "cm_568c67eae410d912c54c"
