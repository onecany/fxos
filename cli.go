...
// the recovery path safe even when FXOS is deployed on the public internet.
//
...
// where `fxos <dbpath>` overrides the SQLite path.
...
// line. Usage: `fxos reset-password --email you@example.com`.
func runResetPassword(args []string) {
	...
	if strings.TrimSpace(*email) == "" {
		...
		fmt.Fprintln(os.Stderr, "usage: fxos reset-password --email you@example.com")
		os.Exit(2)
	}
