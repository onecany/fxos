...
import (
	...
	"net/http"
	"fxos/config"
	fxoscrypto "fxos/crypto"
	"fxos/httpclient"
	"fxos/store"
	hltrader "fxos/trader/hyperliquid"
	"os"
	"strconv"
	"strings"
	"time"
...
func fetchState(wallet string) (*clearinghouseState, error) {
	...
	req, err := http.NewRequest(http.MethodPost, "https://api.hyperliquid.xyz/info", body)
	if err != nil {
		return nil, err
	}
...
func main() {
	_ = godotenv.Load()
	config.Init()
	cryptoService, err := fxoscrypto.NewCryptoService()
	if err != nil {
		panic(err)
	}
	fxoscrypto.SetGlobalCryptoService(cryptoService)
	cfg := config.Get()
