...
func NewStrategyEngine(config *store.StrategyConfig, claw402WalletKey ...string) *StrategyEngine {
	// Create FXOS client with API key from config
	apiKey := config.Indicators.NofxOSAPIKey
	if apiKey == "" {
		apiKey = nofx.DefaultAuthKey
	}
	client := nofx.NewClient(nofx.DefaultBaseURL, apiKey)
	...
	if walletKey != "" {
		...
		claw402Client, err := nofx.NewClaw402DataClient(claw402URL, walletKey, &logger.MCPLogger{})
		if err == nil {
			client.SetClaw402(claw402Client)
			logger.Infof("🔗 FXOS data routed through claw402 (%s)", claw402URL)
		} else {
			logger.Warnf("⚠️ Failed to init claw402 data client: %v (using direct fxosos.ai)", err)
		}
...
func (e *StrategyEngine) FetchNetFlowRankingData() *nofx.NetFlowRankingData {
	...
	if e.usesHyperliquidNativeUniverse() {
		logger.Infof("⏭️  Skipping FXOS netflow ranking for Hyperliquid strategy; native Hyperliquid universe is the source of truth")
		return nil
	}
	...
	data, err := e.fxosClient.GetNetFlowRanking(duration, limit)
	if err != nil {
