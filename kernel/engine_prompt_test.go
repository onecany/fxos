...
// TestModeBlockDefersToCustomEntryStandards locks the mode/entry conflict fix:
// when the operator defined custom Entry Standards (Prompt Studio), the
// built-in Mode block must be demoted to a DEFAULT reference that defers to
// the custom framework. Before the fix, "## Mode: Balanced" read as a hard
// entry rule that contradicted a custom Bollinger system ("entries are
// exclusively generated by Bollinger rules").
func TestModeBlockDefersToCustomEntryStandards(t *testing.T) {
	cfg := store.GetDefaultStrategyConfig("en")
	cfg.CoinSource.SourceType = "static"
	cfg.CoinSource.StaticCoins = []string{"BTCUSDT"}
	cfg.PromptSections.EntryStandards = "## Entry Criteria (Strict)\nEntries are exclusively generated by the Bollinger Band rules defined above."
	engine := NewStrategyEngine(&cfg)
	prompt := engine.BuildSystemPrompt(1000, "balanced")

	if !strings.Contains(prompt, "DEFAULT MODE") {
		t.Fatalf("mode block must be demoted to DEFAULT when custom entry standards exist:\n%s", prompt)
	}
	if !strings.Contains(prompt, "override this entry guidance") {
		t.Fatalf("mode block must explicitly defer to custom entry standards:\n%s", prompt)
	}
}

// TestModeBlockFullWhenNoCustomEntry locks the default path: without custom
// Entry Standards the Mode block must NOT carry the DEFAULT demotion prefix.
func TestModeBlockFullWhenNoCustomEntry(t *testing.T) {
	cfg := store.GetDefaultStrategyConfig("en")
	cfg.CoinSource.SourceType = "static"
	cfg.CoinSource.StaticCoins = []string{"BTCUSDT"}
	engine := NewStrategyEngine(&cfg)
	prompt := engine.BuildSystemPrompt(1000, "balanced")
	if strings.Contains(prompt, "DEFAULT MODE") {
		t.Fatalf("default config must not demote the mode block:\n%s", prompt)
	}
}
