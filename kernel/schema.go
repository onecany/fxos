...
// OIInterpretation defines market interpretations for OI changes
var OIInterpretation = struct {
	OIUp_PriceUp   string
	OIUp_PriceDown string
	OIDown_PriceUp string
	OIDown_PriceDown string
}{
	OIUp_PriceUp:   "Strong bullish trend (new longs opening, capital flowing into long positions)",
	OIUp_PriceDown: "Strong bearish trend (new shorts opening, capital flowing into short positions)",
	OIDown_PriceUp: "Shorts covering (shorts stopped out, potential reversal)",
	OIDown_PriceDown: "Longs closing (longs stopped out, potential reversal)",
}

// ========== Prompt Generation Functions ==========

// GetSchemaPrompt generates schema description text for AI prompts.
// System prompts are intentionally English-only for language-stable
// trading behavior and reproducible model contracts.
func GetSchemaPrompt(lang Language) string {
	return getSchemaPromptEN()
}

// getSchemaPromptEN generates the English schema prompt
func getSchemaPromptEN() string {
	prompt := "# 📖 Data Dictionary & Trading Rules\n\n"
	prompt += "## 📊 Field Definitions\n\n"

	// Fixed iteration order ensures consistent prompt output across runs
	accountMetricsOrder := []string{"Equity", "Balance", "PnL", "Margin"}
	tradeMetricsOrder := []string{"Entry", "Exit", "Profit", "PnL%", "HoldDuration"}
	positionMetricsOrder := []string{"UnrealizedPnL%", "PeakPnL%", "Drawdown", "Leverage", "Margin", "LiqPrice"}
	marketDataOrder := []string{"Volume", "OI", "OIChange"}

	// Account Metrics
	prompt += "### Account Metrics\n"
	for _, key := range accountMetricsOrder {
		if field, ok := DataDictionary["AccountMetrics"][key]; ok {
			prompt += formatFieldDef(key, field)
		}
	}

	// Trade Metrics
	prompt += "\n### Trade Metrics\n"
	for _, key := range tradeMetricsOrder {
		if field, ok := DataDictionary["TradeMetrics"][key]; ok {
			prompt += formatFieldDef(key, field)
		}
	}
	...
	// Position Metrics
	prompt += "\n### Position Metrics\n"
	for _, key := range positionMetricsOrder {
		if field, ok := DataDictionary["PositionMetrics"][key]; ok {
			prompt += formatFieldDef(key, field)
		}
	}
	...
	// Market Data
	prompt += "\n### Market Data\n"
	for _, key := range marketDataOrder {
		if field, ok := DataDictionary["MarketData"][key]; ok {
			prompt += formatFieldDef(key, field)
		}
	}

	// OI Interpretation (with Volume dimension)
	prompt += "\n## 💹 Open Interest (OI) Change Interpretation\n\n"
	prompt += "- **OI Up + Price Up + Volume Up**: Strong bullish trend (new longs opening, high conviction)\n"
	prompt += "- **OI Up + Price Up + Volume Down**: Weak trend (low conviction, possible fakeout)\n"
	prompt += "- **OI Up + Price Down**: Strong bearish trend (new shorts opening)\n"
	prompt += "- **OI Down + Price Up**: Short covering (shorts stopped out, potential reversal)\n"
	prompt += "- **OI Down + Price Down**: Long liquidation (longs stopped out, potential reversal)\n"

	// Funding Rate interpretation (relative to recent average, not absolute thresholds)
	prompt += "\n## 💸 Funding Rate Interpretation\n\n"
	prompt += "- **FR significantly above recent average**: Crowded side (longs if positive, shorts if negative). Contrarian setups become higher quality.\n"
	prompt += "- **FR significantly below recent average**: Opposite side crowded. Trend continuation setups higher quality.\n"
	prompt += "- **FR near zero / at recent average**: Balanced positioning. No crowding signal; rely on other indicators.\n"
	prompt += "- **FR at extreme levels (>5x recent average)**: Highly crowded. Expect mean reversion or squeeze. High risk of sudden reversal.\n"

	return prompt
}

// formatFieldDef formats a field definition (always English)
func formatFieldDef(key string, field BilingualFieldDef) string {
