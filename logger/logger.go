...
func (f *compactFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	...
	for i := 3; i < 10; i++ {
		...
		if !strings.Contains(file, "logrus") && !strings.HasSuffix(file, "logger/logger.go") {
			// Get package name from path (e.g., "fxos/manager/trader_manager.go" -> "manager")
...
func Init(cfg *Config) error {
	...
	if err := os.MkdirAll(logDir, 0755); err == nil {
		logFileName := filepath.Join(logDir, fmt.Sprintf("fxos_%s.log", time.Now().Format("2006-01-02")))
