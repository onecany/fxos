...
// XYZCategory returns the FXOS product category for a Hyperliquid XYZ base symbol.
...
func fetchPerpDexCoins(ctx context.Context, client *http.Client, dex string) ([]CoinInfo, error) {
	...
	resp, err := httpclient.DoJSON(ctx, client, http.MethodPost, hyperliquidInfoURL, reqPayload, nil)
	if err != nil {
...
func GetPerpDexCoins(ctx context.Context, dex string) ([]CoinInfo, error) {
	...
	coins, err := fetchPerpDexCoinsFn(ctx, httpclient.New(30*time.Second), dex)
	if err != nil {
		if hasCache {
