...
// findSafeProxyURL reads proxy configuration from environment variables.
func findSafeProxyURL() *url.URL {
	for _, key := range []string{"ALL_PROXY", "all_proxy"} {
		if v := os.Getenv(key); v != "" {
			if u, err := url.Parse(strings.TrimSpace(v)); err == nil && u != nil {
				return u
			}
		}
	}
...
// SafeHTTPClient returns an HTTP client with SSRF protection and proxy support.
// It validates URLs, blocks requests to private networks, and routes through
// SOCKS5/HTTP proxy when ALL_PROXY, HTTPS_PROXY, or HTTP_PROXY is set.
func SafeHTTPClient(timeout time.Duration) *http.Client {
	baseTransport := httpclient.DefaultTransport()
	baseDialContext := baseTransport.DialContext
	proxyURL := findSafeProxyURL()
	...
	baseTransport.DialContext = func(ctx context.Context, network, addr string) (net.Conn, error) {
		...
		// When a proxy is configured, the transport dials the proxy host and the
		// request URL is validated separately before dispatch / redirect.
		if proxyURL == nil {
	...
	client := httpclient.NewWithTransport(timeout, baseTransport)
