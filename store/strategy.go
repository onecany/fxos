...
type CoinSourceConfig struct {
	...
	// Note: API URLs are now built automatically using FXOS API key from IndicatorConfig
}
...
type IndicatorConfig struct {
	...
	// ========== FXOS Unified API Configuration ==========
	// Unified API Key for all FXOS data sources
	NofxOSAPIKey string `json:"fxosos_api_key,omitempty"`
...
func GetDefaultStrategyConfig(lang string) StrategyConfig {
	...
	config := StrategyConfig{
		...
		Indicators: IndicatorConfig{
			...
			// FXOS datasets do not cover all Hyperliquid XYZ assets, so keep them off.
			NofxOSAPIKey:           "",
	...
	if lang == "zh" {
		config.PromptSections = PromptSectionsConfig{
			RoleDefinition: `# You are the FXOS Claw402 auto-trader
