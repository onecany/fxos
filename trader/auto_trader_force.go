...
func (at *AutoTrader) ensureLongShortCoverage(decisions []kernel.Decision, ctx *kernel.Context, equity float64) []kernel.Decision {
	...
	fill := func(action string, cands []kernel.DirectionalCandidate, have, target int) {
		for _, c := range cands {
			...
			// Skip xyz:/XYZ: symbols on non-Hyperliquid exchanges — these
			// instruments don't exist on CEXes like OKX/Binance/Bybit.
			if at.exchange != "hyperliquid" && strings.HasPrefix(strings.ToUpper(c.Symbol), "XYZ:") {
