...
func (at *AutoTrader) runCycle() error {
	...
	// 9. Populate account snapshot so the decision modal shows real balances
	record.AccountState = store.AccountSnapshot{
		TotalBalance:          ctx.Account.TotalEquity,
		AvailableBalance:      ctx.Account.AvailableBalance,
		TotalUnrealizedProfit: ctx.Account.UnrealizedPnL,
...
func (at *AutoTrader) filterDecisionsToStrategyUniverse(decisions []kernel.Decision, ctx *kernel.Context) []kernel.Decision {
	...
	for _, d := range decisions {
		...
		// Reject xyz:/XYZ: prefixed symbols on non-Hyperliquid exchanges.
		// These are Hyperliquid-only stock/forex perps with no CEX equivalent.
		if at.exchange != "hyperliquid" && strings.HasPrefix(strings.ToUpper(d.Symbol), "XYZ:") {
			if isOpenDecision(d.Action) {
				at.logWarnf("🚫 Blocked AI %s for %s: xyz instruments are only available on Hyperliquid, current exchange is %s", d.Action, d.Symbol, at.exchange)
			} else {
				at.logWarnf("🚫 Dropped AI decision for %s: xyz instruments are only available on Hyperliquid, current exchange is %s", d.Symbol, at.exchange)
			}
			continue
		}
