...
func (at *AutoTrader) executeModifyWithRecord(decision *kernel.Decision, actionRecord *store.DecisionAction) error {
	...
	if decision.StopLoss > 0 {
		if err := at.trader.CancelStopLossOrders(decision.Symbol); err != nil {
			logger.Infof("  ⚠️ Failed to cancel old SL for %s: %v", decision.Symbol, err)
		}
		if err := at.trader.SetStopLoss(decision.Symbol, positionSide, quantity, decision.StopLoss); err != nil {
			logger.Infof("  ❌ Failed to set new SL for %s at %.4f: %v", decision.Symbol, decision.StopLoss, err)
		} else {
			logger.Infof("  ✅ SL updated: %s → %.4f", decision.Symbol, decision.StopLoss)
		}
	}
	...
	if decision.TakeProfit > 0 {
		if err := at.trader.CancelTakeProfitOrders(decision.Symbol); err != nil {
			logger.Infof("  ⚠️ Failed to cancel old TP for %s: %v", decision.Symbol, err)
		}
		if err := at.trader.SetTakeProfit(decision.Symbol, positionSide, quantity, decision.TakeProfit); err != nil {
			logger.Infof("  ❌ Failed to set new TP for %s at %.4f: %v", decision.Symbol, decision.TakeProfit, err)
		} else {
			logger.Infof("  ✅ TP updated: %s → %.4f", decision.Symbol, decision.TakeProfit)
		}
	}
