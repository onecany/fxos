...
func wrapBuilderFeeNotApproved(err error) error {
	if isBuilderFeeNotApprovedError(err) {
		return fmt.Errorf("Hyperliquid builder fee is not approved for FXOS; reconnect Hyperliquid wallet and complete trading authorization: %w", err)
	}
	return err
}
...
func (t *HyperliquidTrader) placeXyzOrder(coin string, isBuy bool, size float64, price float64, reduceOnly bool) (int64, error) {
	...
	// Create OrderAction with FXOS builder fee. Trader startup requires a persisted
