      ...
      */}
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-1">
          ...
          {activeTab === 'kline' && (
            <div className="hidden md:flex items-center gap-1 ml-2 border-l border-[var(--panel-border)] pl-2">
        ...
        {activeTab === 'kline' && (
          <div className="flex items-center gap-2 md:gap-3 w-full md:w-auto min-w-0">
                  ...
                  {showDropdown && (
                    <div className="absolute top-full right-0 mt-2 w-64 bg-fxos-bg-lighter border border-[var(--panel-border)] rounded-lg shadow-[0_10px_40px_-10px_rgba(26,24,19,0.2)] z-50 overflow-hidden fxos-glass ring-1 ring-fxos-gold/10">
                      <div className="p-2 border-b border-[var(--panel-border)]">
                        <div className="flex items-center gap-2 px-2 py-1.5 bg-fxos-bg-deeper rounded border border-[var(--panel-border)] focus-within:border-fxos-gold/50 transition-colors">
                          <Search className="w-3.5 h-3.5 text-fxos-text-muted" />
                          <input
                            type="text"
                            value={searchFilter}
                            onChange={(e) => setSearchFilter(e.target.value)}
                            placeholder="Search symbol..."
                            className="flex-1 bg-transparent text-[11px] text-fxos-text placeholder-fxos-text-muted focus:outline-none font-mono"
                            autoFocus
                          />
                        </div>
                      </div>
            ...
            <div className="flex items-center bg-fxos-bg-deeper rounded border border-[var(--panel-border)] overflow-x-auto no-scrollbar max-w-[200px] md:max-w-none">
              {INTERVALS.map((int) => (
                <button
                  key={int.value}
      ...
      <div className="relative flex-1 bg-fxos-bg/50 rounded-b-lg overflow-hidden h-full min-h-0">
