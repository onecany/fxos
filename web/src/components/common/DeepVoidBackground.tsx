...
export function DeepVoidBackground({ children, className = '', disableAnimation = false, ...props }: DeepVoidBackgroundProps) {
    return (
        <div className={`relative w-full min-h-screen bg-fxos-bg text-fxos-text overflow-hidden flex flex-col ${className}`} {...props}>
            {disableAnimation ? (
                <>
                ...
                <>
                    ...
                    <div className="absolute inset-0 pointer-events-none z-0">
                        <div className="absolute inset-x-0 bottom-0 h-[55vh] bg-[linear-gradient(to_right,rgba(45,212,191,0.10)_1px,transparent_1px),linear-gradient(to_bottom,rgba(45,212,191,0.10)_1px,transparent_1px)] bg-[size:40px_40px] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_0%,#000_70%,transparent_100%)] opacity-50" style={{ transform: 'perspective(500px) rotateX(62deg) translateY(100px) scale(2)' }}></div>
                    </div>
                </>
            )}
