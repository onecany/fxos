    ...
    <nav className="fixed top-0 w-full z-50 header-bar">
      <div className="flex items-center justify-between h-16 px-4 sm:px-6 max-w-[1920px] mx-auto">
              ...
              return navTabs
                ...
                .map((tab) => (
                  <button
                    ...
                    ${resolvedCurrentPage === tab.page ? 'text-fxos-gold' : 'text-fxos-text-muted hover:text-fxos-gold'}`}
                  >
                    {resolvedCurrentPage === tab.page && (
                      <span className="absolute inset-0 rounded-lg bg-fxos-gold/15 -z-10" />
                    )}
                    {tab.label}
                    {tab.badge && (
                      <span className="ml-1 text-[10px] px-1.5 py-0.5 rounded-full bg-fxos-gold/20 text-fxos-gold font-semibold uppercase align-top relative -top-1">
                        {tab.badge}
                      </span>
                    )}
                  </button>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-1">
              ...
              <a
                ...
                className="p-2 rounded-lg transition-all hover:scale-110 text-fxos-text-muted hover:text-fxos-text hover:bg-fxos-gold-dim"
                title="GitHub"
              >
                <svg
                  width="18"
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="p-2 rounded-lg transition-all hover:scale-110 text-fxos-text-muted hover:text-[#0088cc] hover:bg-[#0088cc]/10"
                title="Telegram"
              >
                <svg
                  width="16"
            ...
            {/* Theme Toggle — one-click dark/light sci-fi flip */}
            <button
              type="button"
              onClick={toggleTheme}
              aria-label={
              ...
              className="relative flex items-center justify-center w-9 h-9 rounded-lg border border-fxos-gold/25 bg-fxos-bg-lighter text-fxos-text-muted hover:text-fxos-gold hover:border-fxos-gold/50 hover:shadow-[0_0_14px_rgba(45,212,191,0.25)] transition-all"
            >
              {theme === 'dark' ? (
            ...
            {/* Divider */}
            <div className="h-5 w-px bg-fxos-text/15" />
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="relative" ref={userDropdownRef}>
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-2 rounded transition-colors bg-fxos-bg-lighter border border-fxos-gold/20 hover:bg-fxos-gold-dim"
                  >
                    <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold bg-fxos-gold text-white">
                  ...
                  {userDropdownOpen && (
                    <div className="absolute right-0 top-full mt-2 w-48 rounded-lg shadow-lg overflow-hidden z-50 bg-fxos-bg-lighter border border-fxos-gold/20">
                      <div className="px-3 py-2 border-b border-fxos-gold/20">
                        <div className="text-xs text-fxos-text-muted">
                          {t('loggedInAs', language)}
                        </div>
                        <div className="text-sm font-medium text-fxos-text-muted">
                          {user.email}
                        </div>
                      </div>
                      <button
                        ...
                        className="w-full flex items-center gap-2 px-3 py-2 text-sm transition-colors hover:bg-fxos-gold-dim text-fxos-text-muted hover:text-fxos-text"
                      >
                      ...
                      {onLogout && (
                        <button
                          ...
                          className="w-full px-3 py-2 text-sm font-semibold transition-colors hover:opacity-80 text-center bg-fxos-danger/20 text-fxos-danger"
                        >
              ...
              resolvedCurrentPage !== 'register' && (
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="px-3 py-2 text-sm font-medium transition-colors rounded text-fxos-text-muted hover:text-fxos-text"
                  >
                    {t('signIn', language)}
                  </button>
                </div>
        ...
        <motion.button
          ...
          className="md:hidden text-fxos-text-muted hover:text-fxos-text"
          whileTap={{ scale: 0.9 }}
        >
          {mobileMenuOpen ? (
            <X className="w-6 h-6" />
          ...
          >
            ...
            >
                  ...
                  return navTabs
                    ...
                    .map((tab, i) => (
                      <motion.button
                        ...
                        ${resolvedCurrentPage === tab.page ? 'text-fxos-gold' : 'text-zinc-500'}`}
                      >
                        {resolvedCurrentPage === tab.page && (
                          <motion.div
                            layoutId="active-indicator"
                            className="w-1.5 h-1.5 rounded-full bg-fxos-gold"
                          />
                        )}
                        {tab.label}
                        {tab.badge && (
                          <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-fxos-gold/20 text-fxos-gold font-semibold uppercase align-middle relative -top-1">
                            {tab.badge}
                          </span>
                        )}
              ...
              <div className="mt-auto space-y-8">
                {/* Language Switcher */}
                <div>
                  <div className="text-xs text-zinc-600 uppercase tracking-widest mb-3 font-mono">
                ...
                <div>
                  ...
                  <button
                    type="button"
                    onClick={toggleTheme}
                    className="w-full flex items-center justify-between gap-3 px-4 py-3 rounded-lg border border-fxos-gold/25 bg-fxos-bg-lighter text-fxos-text hover:text-fxos-gold transition-colors"
                  >
                    <span className="text-sm font-semibold">
                      {theme === 'dark' ? 'Switch to Light' : 'Switch to Dark'}
                    </span>
                    {theme === 'dark' ? (
                      <Sun className="w-4 h-4 text-fxos-gold" />
                    ) : (
                      <Moon className="w-4 h-4 text-fxos-gold" />
                    )}
                  </button>
                </div>
                ...
                <div className="flex items-center gap-4">
                  {[
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-12 h-12 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-500 hover:text-fxos-gold hover:border-fxos-gold transition-colors"
                    >
                      <svg
                        width="20"
                        height="20"
                  ...
                  {isLoggedIn && user ? (
                    ...
                    resolvedCurrentPage !== 'register' && (
                      <button
                        ...
                        className="flex items-center justify-center bg-fxos-gold text-white rounded-lg font-bold text-sm hover:opacity-90 transition-colors"
                      >
                        {t('signIn', language)}
                      </button>
                    )
...
// Inline language switcher for the header bar
function LanguageSwitcherInline() {
  ...
  return (
    <div className="flex items-center gap-1 rounded-lg p-1 border border-[var(--panel-border)] bg-fxos-bg-lighter">
