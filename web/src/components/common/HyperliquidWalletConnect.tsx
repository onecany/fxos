...
const STORAGE_KEY = 'fxos.hyperliquid.connection.v6'
const AGENT_NAME = 'FXOS Agent'
...
// this suffix from the stored/displayed name, so the named slot stays "FXOS Agent".
...
function walletSupportLabel(language: Language) {
  return t('hlWalletSupport', language)
}
...
export function HyperliquidWalletConnect({
  ...
  const text = useMemo(
    () => ({
      ...
      connect: t('hlConnect', language),
      connected: t('hlConnected', language),
      mainWallet: t('hlConnectWallet', language),
      generateAgent: t('hlCreateKey', language),
      approveAgent: t('hlApproveAgent', language),
      approveBuilder: t('hlApproveBuilder', language),
      save: t('hlSaveFxos', language),
      done: t('hlAllSet', language),
      balance: t('hlBalance', language),
      withdrawable: t('hlWithdrawable', language),
      equity: t('hlEquity', language),
      marginUsed: t('hlMarginUsed', language),
      unrealizedPnl: t('hlUnrealizedPnl', language),
      refresh: t('refreshBtn', language),
      noCustody: t('hlNoCustody', language),
      agentExpiry: t('hlAgentExpiry', language),
      agentExpired: t('hlExpired', language),
      agentNoAuth: t('hlNoAuth', language),
      renewAgent: t('hlRenewAgent', language),
      renewHint: t('hlRenewHint', language),
      noWalletTitle: t('hlNoWalletTitle', language),
      noWalletDetail: t('hlNoWalletDetail', language),
      installRabby: t('installRabby', language),
      installMetaMask: t('hlInstallMetaMask', language),
    }),
    [language]
  )
    ...
    try {
      ...
      const first =
        Array.isArray(accounts) && typeof accounts[0] === 'string'
          ? accounts[0]
          : ''
      if (!first) throw new Error(t('hlWalletReturnedNoAccount', language))
  ...
  async function approveAgent() {
    ...
    if (!state.agentAddress) {
      ...
      setError(t('hlGenerateBeforeSave', language))
      return
    }
    setBusy(true)
    try {
  ...
  async function renewAgentAuthorization() {
    ...
    try {
      ...
      // Hold the new agent + key so the manual "Save to FXOS" button can recover
      ...
      if (existing) {
        ...
        toast.success(
          state.agentPrivateKey
            ? t('hlAccountUpdated', language)
            : t('hlExistingAuthUpdated', language)
        )
        await onSaved?.()
        return
      }
  ...
  return (
    <div className={inline ? 'relative w-full' : 'relative'}>
        ...
        <button
          ...
          className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-bold transition-all border ${
            connectedAddr
              ...
              : 'bg-fxos-gold/10 border-fxos-gold/30 text-fxos-gold hover:bg-fxos-gold/20'
          }`}
        >
      ...
      {(open || inline) && (
        <div
          className={`${inline ? 'relative w-full' : 'absolute right-0 top-full mt-2 w-[420px] shadow-2xl shadow-black/10'} rounded-2xl border border-[var(--panel-border)] bg-fxos-bg-lighter z-[80] overflow-hidden`}
        >
          <div className="flex items-center justify-between p-4 border-b border-[var(--panel-border)]">
            <div>
              <div className="font-bold text-fxos-text">{text.title}</div>
              <div className="text-xs text-fxos-text-muted mt-1">
                {text.noCustody}
              </div>
              <div className="text-[11px] text-fxos-gold/80 mt-1">
            ...
            {!inline && (
              <button
                ...
                className="p-1 rounded hover:bg-fxos-gold-dim text-fxos-text-muted"
              >
                <X className="w-4 h-4" />
              </button>
            )}
          ...
          <div className="p-4 space-y-4">
                ...
                <div key={step.key} className="flex items-center gap-3 text-sm">
                  <div
                    className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                      step.status === 'done'
                        ? 'bg-fxos-success text-white'
                        : step.status === 'active'
                          ? 'bg-fxos-gold text-white'
                          : 'bg-fxos-bg-deeper text-fxos-text-muted'
                    }`}
                  >
                    {step.status === 'done' ? (
            ...
            {error && (
              <div className="rounded-lg border border-fxos-danger/30 bg-fxos-danger/10 p-3 text-xs text-fxos-danger">
                {error}
              </div>
            )}

            {!state.mainWallet && !hasWalletProvider && (
              <div className="rounded-xl border border-fxos-gold/20 bg-fxos-gold/5 p-3">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-2 rounded-lg border border-[var(--panel-border)] bg-fxos-bg-deeper px-3 py-2 text-xs font-semibold text-fxos-text hover:border-[var(--panel-border)] hover:bg-fxos-bg"
                  >
            ...
            {agentInfo && (
              <div className="rounded-xl border border-fxos-gold/20 bg-fxos-gold/5 p-3 space-y-2 text-xs">
                <button
                  type="button"
                  disabled={busy}
                  onClick={renewAgentAuthorization}
                  className="w-full flex items-center justify-center gap-2 rounded-xl border border-fxos-gold/30 bg-fxos-gold/10 px-4 py-2.5 text-sm font-bold text-fxos-gold transition hover:bg-fxos-gold/20 disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  {busy ? (
                ...
                <p className="text-[11px] leading-relaxed text-fxos-text-muted">
                  {text.renewHint}
                </p>
              </div>
            )}
              ...
              <div className="rounded-xl border border-fxos-gold/20 bg-fxos-gold/5 p-3 space-y-3 text-xs">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-fxos-text-muted hover:text-fxos-gold disabled:opacity-60"
                  >
                    <RefreshCw
                ...
                {balanceError ? (
                  <div className="rounded-lg border border-fxos-danger/30 bg-fxos-danger/10 p-2 text-fxos-danger">
                    {balanceError}
                  </div>
                ) : (
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="rounded-lg bg-fxos-bg-deeper p-2">
                      <div className="text-fxos-text-muted">{text.equity}</div>
            ...
            <div className="grid grid-cols-1 gap-2">
              ...
              {complete && (
                <>
                  <div className="rounded-lg border border-fxos-success/30 bg-fxos-success/10 p-3 text-sm text-fxos-success flex items-center gap-2">
                    <Shield className="w-4 h-4" /> {text.done}
                  </div>
                  <button
                    type="button"
                    onClick={resetTradingAuthorization}
                    className="w-full flex items-center justify-center gap-2 rounded-xl border border-fxos-gold/30 bg-fxos-gold/10 px-4 py-3 text-sm font-bold text-fxos-gold transition hover:bg-fxos-gold/20"
                  >
                    {t('hlReauthorize', language)}
                  </button>
                </>
              )}
            </div>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-[var(--panel-border)]">
              ...
              <button
                type="button"
                onClick={resetFlow}
                className="text-xs text-fxos-text-muted hover:text-fxos-danger"
              >
                {t('hlReset', language)}
              </button>
            </div>
...
function ActionButton({
  ...
  return (
    <button
      ...
      disabled={busy}
      onClick={onClick}
      className="w-full flex items-center justify-center gap-2 rounded-xl bg-fxos-gold px-4 py-3 text-sm font-bold text-white transition hover:opacity-90 disabled:opacity-60 disabled:cursor-not-allowed"
    >
