...
export function WebCryptoEnvironmentCheck({
  ...
  const statusRendererMap: Record<WebCryptoCheckStatus, () => ReactNode> = {
    secure: () => (
      <div className="flex items-start gap-2 text-fxos-success text-xs">
    ...
    unsupported: () => (
      <div className="text-xs" style={{ color: 'var(--fxos-danger)' }}>
        <div className="flex items-start gap-2 mb-1">
          <ShieldAlert className="w-4 h-4 flex-shrink-0" />
          <div className="font-semibold">
    ...
    disabled: () => (
      <div className="flex items-start gap-2 text-fxos-text-muted text-xs">
