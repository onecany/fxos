...
import { faqZhCategories } from './faqDataZh'
import { faqIdCategories } from './faqDataId'
...
/** Localized tree shape — same as FAQCategory without the icon. */
export type FAQCategoryContent = Omit<FAQCategory, 'icon'>

/** Category icons keyed by category id (shared across languages). */
export const faqCategoryIcons: Record<string, LucideIcon> = {
...
export function getFaqCategories(language: string): FAQCategory[] {
  ...
  return tree.map((category) => ({
    ...category,
    icon: faqCategoryIcons[category.id] || BookOpen,
  }))
}
...
export const faqCategories: FAQCategory[] = [
  // ───────────────────────── Getting started ─────────────────────────
  {
    ...
    items: [
      {
        id: 'what-is-fxos',
        question: 'What is FXOS?',
        blocks: [
          {
            type: 'p',
            text: 'FXOS is an open-source, self-hosted AI trading terminal. Its flagship mode is the FXOS Autopilot: an AI agent that reads the Claw402.ai signal board, verifies candidates with Signal Lab and liquidation structure, confirms timing with raw candles, and executes on Hyperliquid — all on your own machine, with your keys never leaving your server.',
          },
          {
            type: 'p',
      ...
      },
      {
        ...
        blocks: [
          {
            type: 'p',
            text: 'No API keys are required. FXOS routes inference through Claw402 pay-as-you-go infrastructure: your AI fee wallet pays per call with Base USDC, and the terminal accesses supported models (DeepSeek and other frontier models) on demand.',
          },
          {
            type: 'p',
      ...
      },
      {
        ...
        blocks: [
          ...
          {
            type: 'note',
            text: 'Trading involves substantial risk of loss. FXOS is software, not investment advice.',
          },
        ],
      },
    ],
    ...
    items: [
      ...
      },
      {
        ...
        blocks: [
          {
            type: 'p',
            text: 'FXOS uses Hyperliquid agent wallets, so your main wallet key is never shared. The connect flow has four signed steps:',
          },
          {
            type: 'steps',
            items: [
              'Connect your EVM wallet (Rabby, MetaMask, OKX, Coinbase Wallet).',
              'Approve a freshly generated FXOS agent wallet — valid for 180 days, trading only.',
              'Approve the builder fee (a small per-order fee that funds the platform).',
              'Save the agent key to your FXOS server (stored encrypted).',
            ],
          },
          {
            type: 'p',
      ...
      },
      {
        ...
        blocks: [
          {
            type: 'p',
            text: 'Launching is idempotent. If a FXOS Autopilot already exists, the launcher updates it with the current strategy config and restarts it — it never creates a duplicate. A restart can take up to a minute if the bot is mid-cycle; the UI waits for it.',
          },
        ],
      },
      {
        ...
        blocks: [
          {
            ...
            items: [
              ...
              'Emergencies: you can always manage positions directly on Hyperliquid; FXOS never locks you out of your own account.',
            ],
          },
        ],
      },
  ...
  // ───────────────────────── Security ─────────────────────────
  {
    ...
    items: [
        ...
        blocks: [
          {
            ...
            items: [
              ...
              'FXOS is self-hosted: nothing is sent to any third-party server. The code is open source and auditable.',
            ],
          },
        ],
      },
      {
        ...
        question: 'Can FXOS withdraw or steal my funds?',
        blocks: [
          {
            type: 'p',
            text: 'No. On Hyperliquid, FXOS only ever holds an agent wallet, which by protocol design can trade but cannot withdraw. Your collateral stays in your own account under your main wallet’s control.',
          },
          {
            type: 'note',
  ...
  // ───────────────────────── Self-hosting ─────────────────────────
  {
    ...
    items: [
      {
        id: 'how-to-install',
        question: 'How do I install FXOS?',
        blocks: [
          {
            type: 'p',
        ...
        blocks: [
          {
            type: 'list',
            items: [
              'Backend: `docker logs fxos-trading` (or the terminal running `go run .`).',
    ...
    items: [
        ...
        blocks: [
          ...
          },
          {
            ...
            items: [
              ...
              'Open a PR against `onecany/fxos:dev`, reference the issue (`Closes #123`), and attach screenshots for UI changes.',
            ],
          },
        ],
      },
      ...
      },
      {
        ...
        blocks: [
          {
            type: 'p',
            text: 'Open a GitHub issue with the template: what you did, what happened, backend logs (`docker logs fxos-trading`), and screenshots. For suspected security issues, please follow the responsible-disclosure notes in SECURITY.md instead of a public issue.',
          },
          {
            type: 'links',
            links: [
