...
const generateLog = (id: number, language: Language): LogEntry => {
  ...
  switch (type) {
    ...
    case 'SIGNAL':
      ...
      break
    case 'RISK':
      msg = t('landing.logRisk', language, {
        pair: pairs[Math.floor(Math.random() * pairs.length)],
      })
      color = 'text-fxos-danger'
      break
    case 'MACRO':
      msg = t('landing.logMacro', language, {
        ...
        <div className="flex-1 overflow-hidden relative font-mono text-[10px] md:text-sm h-full flex items-center">
              ...
              >
                ...
                <span
                  className={`font-bold w-10 ${
                    log.type === 'RISK'
                      ? 'text-fxos-danger bg-fxos-danger/10 px-1 rounded'
                      : log.type === 'SIGNAL'
                        ? 'text-fxos-gold bg-fxos-gold/10 px-1 rounded'
                        : log.type === 'EXEC'
                          ? 'text-fxos-success'
          ...
          <div className="md:hidden flex flex-col gap-2 w-full p-4 h-full overflow-hidden">
            {logs.map((log) => (
              <div
                key={log.id}
                className="flex gap-2 w-full truncate border-b border-[var(--panel-border)] pb-1 last:border-0"
              >
                <span className="text-fxos-text-muted w-16 shrink-0">
                  {log.time.split('.')[0]}
                </span>
                <span
