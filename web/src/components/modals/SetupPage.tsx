...
const labels = {
  ...
  id: {
    welcome: 'Selamat Datang di FXOS',
...
export function SetupPage() {
  ...
  useEffect(() => {
    ...
    localStorage.removeItem('fxos_beginner_onboarding_completed')
    localStorage.removeItem('fxos_beginner_wallet_address')
  }, [])
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-fxos-bg">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[10%] left-[15%] w-[500px] h-[500px] bg-fxos-gold/8 rounded-full blur-[150px]" />
        <div className="absolute bottom-[5%] right-[10%] w-[400px] h-[400px] bg-fxos-gold/6 rounded-full blur-[140px]" />
        <div className="absolute top-[40%] right-[30%] w-[300px] h-[300px] bg-fxos-success/4 rounded-full blur-[120px]" />
      </div>

      {/* Faux UI elements in background to simulate the app */}
      <div className="absolute inset-0 pointer-events-none opacity-[0.06]">
        {/* Fake header bar */}
        <div className="h-14 border-b border-fxos-text/20 flex items-center px-6 gap-4">
          <div className="w-8 h-8 rounded-lg bg-fxos-text/40" />
          <div className="h-3 w-20 rounded bg-fxos-text/30" />
          <div className="h-3 w-16 rounded bg-fxos-text/20 ml-4" />
          <div className="h-3 w-16 rounded bg-fxos-text/20" />
          <div className="h-3 w-16 rounded bg-fxos-text/20" />
        </div>
        {/* Fake content cards */}
        ...
        <div className="px-6 mt-2">
          <div className="h-64 rounded-xl border border-fxos-text/15 bg-fxos-text/5" />
        </div>
      </div>
      ...
      <div className="relative z-10 flex min-h-screen items-center justify-center px-4 py-16">
        <div className="w-full max-w-sm animate-[fadeInUp_0.4s_ease-out]">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-fxos-bg-deeper border border-[var(--panel-border)] rounded-xl px-4 py-3 text-sm text-fxos-text placeholder-fxos-text-muted focus:outline-none focus:border-fxos-gold/60 focus:ring-1 focus:ring-fxos-gold/30 transition-all"
              ...
              {/* Error */}
              {error && (
                <p className="text-xs text-fxos-danger bg-fxos-danger/10 border border-fxos-danger/20 rounded-lg px-3 py-2">
                  {error}
                </p>
              )}
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-fxos-gold hover:bg-fxos-gold-highlight active:scale-[0.98] text-fxos-bg font-semibold py-3 rounded-xl text-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed mt-2"
              >
          ...
          <p className="text-center text-xs text-fxos-text-muted mt-6">
            {l.singleUser}
          </p>
        </div>
      </div>
