    ...
    try {
      ...
      if (navigator.clipboard) {
        try {
          ...
          toast.success(t('obfuscationCopied', language))
        } catch {
          ...
          toast.error(t('obfuscationCopyFailed', language))
        }
      } else {
        setClipboardStatus('failed')
        setObfuscationLog([
  ...
  const modalContent = useMemo(() => {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
        <div className="bg-fxos-bg-lighter p-8 rounded-xl max-w-lg w-full mx-4 border border-[var(--panel-border)]">
          <div className="text-center mb-6">
            <h2 className="text-xl font-bold text-fxos-text mb-2">
              🔐 {t('twoStageKey.title', language)}
              {contextLabel && (
                <span className="text-fxos-text-muted text-base font-normal ml-2">
                  ({contextLabel})
                </span>
              )}
            </h2>
          ...
          {stage === 1 && (
            <div className="space-y-4">
              <div>
                <label className="block text-fxos-text-muted text-sm mb-2">
                ...
                <input
                  ...
                  placeholder="0x1234..."
                  className="w-full bg-fxos-bg-deeper border border-[var(--panel-border)] rounded-lg px-4 py-3 text-fxos-text font-mono text-sm focus:border-fxos-gold focus:outline-none"
                  maxLength={expectedPart1Length + 2} // +2 for optional 0x prefix
                  disabled={processing}
                />
              </div>

              {error && <div className="text-fxos-danger text-sm">{error}</div>}
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 bg-fxos-gold hover:bg-fxos-gold-highlight disabled:bg-fxos-bg-deeper disabled:text-fxos-text-muted text-fxos-bg font-medium py-3 px-4 rounded-lg transition-colors"
                >
                  {processing
                ...
                <button
                  onClick={onCancel}
                  disabled={processing}
                  className="px-6 py-3 text-fxos-text-muted hover:text-fxos-text border border-[var(--panel-border)] rounded-lg transition-colors"
                >
          ...
          {stage === 2 && clipboardStatus !== 'idle' && (
            <div className="mb-4 p-4 rounded-lg bg-fxos-gold/10 border border-fxos-gold/40">
              {clipboardStatus === 'copied' && (
                <div className="text-fxos-text">
              ...
              {clipboardStatus === 'failed' && manualObfuscationValue && (
                <div className="text-fxos-gold">
                  <div className="font-medium">
                    {t('twoStageKey.obfuscationManual', language)}
                  </div>
                  <div className="text-xs mt-2 p-2 bg-fxos-bg-deeper rounded font-mono break-all border border-[var(--panel-border)]">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 bg-fxos-success hover:bg-fxos-success/90 disabled:bg-fxos-bg-deeper disabled:text-fxos-text-muted text-fxos-bg font-medium py-3 px-4 rounded-lg transition-colors"
                >
