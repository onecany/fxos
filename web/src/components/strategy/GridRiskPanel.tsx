...
export function GridRiskPanel({
  ...
  const getRegimeColor = (regime: string) => {
    switch (regime) {
      case 'narrow': return 'var(--fxos-success)'
      case 'standard': return 'var(--fxos-gold)'
      case 'wide': return 'var(--fxos-gold)'
      case 'volatile': return 'var(--fxos-danger)'
      case 'trending': return 'var(--fxos-gold)'
  ...
  const getBreakoutColor = (level: string) => {
    switch (level) {
      case 'none': return 'var(--fxos-success)'
      case 'short': return 'var(--fxos-gold)'
      case 'mid': return 'var(--fxos-gold)'
      case 'long': return 'var(--fxos-danger)'
      default: return 'var(--text-secondary)'
    }
  }
  ...
  if (error) {
    return (
      <div className="p-3 text-center text-xs" style={{ color: 'var(--fxos-danger)' }}>
      ...
      {expanded && (
        <div className="px-3 pb-3 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {/* Leverage */}
            <div className="p-2 rounded" style={{ background: 'var(--fxos-bg-lighter)' }}>
              <div className="flex items-center gap-1 mb-2">
                <TrendingUp className="w-3 h-3" style={{ color: 'var(--fxos-gold)' }} />
                <span className="text-xs font-medium" style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.leverageInfo, language)}</span>
              </div>
              <div className="grid grid-cols-3 gap-1 text-xs">
                <div>
                  <div style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.currentLeverage, language)}</div>
                  <div className="font-mono" style={{ color: 'var(--text-primary)' }}>{riskInfo.current_leverage}x</div>
                </div>
                <div>
                  <div style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.effectiveLeverage, language)}</div>
                  <div className="font-mono" style={{ color: 'var(--fxos-gold)' }}>{riskInfo.effective_leverage.toFixed(2)}x</div>
                </div>
                <div>
                  <div style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.recommendedLeverage, language)}</div>
                  <div
                    className="font-mono"
                    style={{ color: riskInfo.current_leverage > riskInfo.recommended_leverage ? 'var(--fxos-danger)' : 'var(--fxos-success)' }}
                  >
            ...
            <div className="p-2 rounded" style={{ background: 'var(--fxos-bg-lighter)' }}>
              <div className="flex items-center gap-1 mb-2">
                <Activity className="w-3 h-3" style={{ color: 'var(--fxos-gold)' }} />
                <span className="text-xs font-medium" style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.positionInfo, language)}</span>
              </div>
              <div className="grid grid-cols-3 gap-1 text-xs">
                <div>
                  <div style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.currentPosition, language)}</div>
                  <div className="font-mono" style={{ color: 'var(--text-primary)' }}>{formatUSD(riskInfo.current_position)}</div>
                </div>
                <div>
                  <div style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.maxPosition, language)}</div>
                  <div className="font-mono" style={{ color: 'var(--text-primary)' }}>{formatUSD(riskInfo.max_position)}</div>
                </div>
                <div>
                  <div style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.positionPercent, language)}</div>
              ...
              {/* Mini progress bar */}
              <div className="h-1 mt-2 rounded-full overflow-hidden" style={{ background: 'var(--fxos-bg-lighter)' }}>
                <div
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="p-2 rounded" style={{ background: 'var(--fxos-bg-lighter)' }}>
              <div className="flex items-center gap-1 mb-2">
                <Shield className="w-3 h-3" style={{ color: 'var(--fxos-gold)' }} />
                <span className="text-xs font-medium" style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.marketState, language)}</span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  <div style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.regimeLevel, language)}</div>
                  <div className="font-medium" style={{ color: getRegimeColor(riskInfo.regime_level) }}>
                    {ts(gridRisk[(riskInfo.regime_level || 'standard') as keyof typeof gridRisk], language)}
                  </div>
                </div>
                <div>
                  <div style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.currentPrice, language)}</div>
                  <div className="font-mono" style={{ color: 'var(--text-primary)' }}>{formatPrice(riskInfo.current_price)}</div>
                </div>
                <div>
                  <div style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.breakoutLevel, language)}</div>
                ...
                <div>
                  <div style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.breakoutDirection, language)}</div>
                  <div
                    className="font-medium"
                    style={{ color: riskInfo.breakout_direction === 'up' ? 'var(--fxos-success)' : riskInfo.breakout_direction === 'down' ? 'var(--fxos-danger)' : 'var(--text-secondary)' }}
                  >
            ...
            <div className="p-2 rounded" style={{ background: 'var(--fxos-bg-lighter)' }}>
              <div className="flex items-center gap-1 mb-2">
                <AlertTriangle className="w-3 h-3" style={{ color: 'var(--fxos-danger)' }} />
                <span className="text-xs font-medium" style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.liquidationInfo, language)}</span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  <div style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.liquidationPrice, language)}</div>
                  <div className="font-mono" style={{ color: 'var(--fxos-danger)' }}>
                    {riskInfo.liquidation_price > 0 ? formatPrice(riskInfo.liquidation_price) : '-'}
                  </div>
                </div>
                <div>
                  <div style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.liquidationDistance, language)}</div>
                  <div className="font-mono" style={{ color: 'var(--fxos-danger)' }}>
          ...
          <div className="p-2 rounded" style={{ background: 'var(--fxos-bg-lighter)' }}>
            <div className="flex items-center gap-1 mb-2">
              ...
              <span className="text-xs font-medium" style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.boxState, language)}</span>
            </div>
            <div className="grid grid-cols-3 gap-2 text-xs">
              <div className="flex justify-between">
                <span style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.shortBox, language)}</span>
                <span className="font-mono" style={{ color: 'var(--text-primary)' }}>
                  {formatPrice(riskInfo.short_box_lower)} - {formatPrice(riskInfo.short_box_upper)}
                </span>
              </div>
              <div className="flex justify-between">
                <span style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.midBox, language)}</span>
                <span className="font-mono" style={{ color: 'var(--text-primary)' }}>
                  {formatPrice(riskInfo.mid_box_lower)} - {formatPrice(riskInfo.mid_box_upper)}
                </span>
              </div>
              <div className="flex justify-between">
                <span style={{ color: 'var(--text-secondary)' }}>{ts(gridRisk.longBox, language)}</span>
                <span className="font-mono" style={{ color: 'var(--text-primary)' }}>
