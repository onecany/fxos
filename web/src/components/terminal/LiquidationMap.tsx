...
const C_LONG_LIQ = '#d4a017' // gold — long-liquidation fuel (price falls), brighter than cost green
const C_SHORT_LIQ = '#3498db' // sky blue — short-liquidation fuel (price rises), brighter than cost red
...
export function LiquidationMap({ symbol, marketType = 'hip3_perp', height = 460, demo = false }: LiquidationMapProps) {
  ...
  const rowH = view.rows.length > 44 ? 11 : view.rows.length > 28 ? 14 : 18
  const hv = hover != null ? view.rows[hover] : null

  return (
    <div style={{ fontFamily: 'var(--tm-mono)' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 3 }}>
        <span className="tm-px" style={{ fontSize: 11 }}>{t('costLiqMap', language)}</span>
        ...
        <span className="tm-sc" style={{ marginLeft: 'auto', color: view.rows.length ? 'var(--tm-up)' : 'var(--tm-muted)' }}>
          {view.rows.length ? t('costLiqMapLive', language) : isLoading ? t('costLiqMapSync', language) : '○ —'}
        </span>
      </div>

      {/* legend */}
      <div className="tm-sc" style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 4, fontSize: 9 }}>
        <Swatch c={C_LONG_COST} label={t('longCost', language)} />
        <Swatch c={C_SHORT_COST} label={t('shortCost', language)} />
        <Swatch c={C_LONG_LIQ} label={t('longLiq', language)} />
        <Swatch c={C_SHORT_LIQ} label={t('shortLiq', language)} />
      </div>
      ...
      <div className="tm-mono" style={{ fontSize: 10, color: 'var(--tm-ink-2)', minHeight: 14, marginBottom: 2 }}>
        {hv ? (
          <span>
            <b>{fmtPx(hv.px)}</b> · {t('costLine', language)} <span style={{ color: C_LONG_COST }}>{fmtUsd(hv.longCost)}</span>/<span style={{ color: C_SHORT_COST }}>{fmtUsd(hv.shortCost)}</span>
            {' · '}liq <span style={{ color: C_LONG_LIQ }}>{fmtUsd(hv.longLiq)}</span>/<span style={{ color: C_SHORT_LIQ }}>{fmtUsd(hv.shortLiq)}</span>
          </span>
        ) : (
          <span className="tm-sc">{t('markPrice', language)} <b style={{ color: 'var(--tm-red)' }}>{view.mark ? fmtPx(view.mark) : '—'}</b> · {view.costAddrs.toLocaleString()} {t('positionsLabel', language)} / {view.liqAddrs.toLocaleString()} {t('liqLevels', language)}</span>
        )}
      </div>

      {error && !view.rows.length ? (
        <div className="tm-sc" style={{ padding: '16px 0' }}>{t('noHeatmap', language)} {view.dispSymbol}</div>
      ) : !view.rows.length ? (
        <div className="tm-sc" style={{ padding: '16px 0' }}>{t('loadingHeatmap', language)}</div>
      ) : (
        <div>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  alignItems: 'center',
                  height: rowH,
                  background: hover === i ? 'color-mix(in srgb, var(--tm-ink) 6%, transparent)' : 'transparent',
          ...
          <div className="tm-sc" style={{ display: 'flex', gap: 10, marginTop: 4, fontSize: 9, flexWrap: 'wrap' }}>
            <span>{t('costLine', language)} <span style={{ color: C_LONG_COST }}>{fmtUsd(view.totals.lc)}</span>/<span style={{ color: C_SHORT_COST }}>{fmtUsd(view.totals.sc)}</span></span>
            <span>{t('liqLevelsLabel', language)} <span style={{ color: C_LONG_LIQ }}>{fmtUsd(view.totals.ll)}</span>/<span style={{ color: C_SHORT_LIQ }}>{fmtUsd(view.totals.sl)}</span></span>
          </div>
        </div>
      )}
    </div>
