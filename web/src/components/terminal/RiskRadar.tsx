...
export function RiskRadar({ positions, account, config, fullStats }: RiskRadarProps) {
  ...
  if (!hasData) {
    return <div className="tm-sc" style={{ padding: '16px 0' }}>{language === 'zh' ? '暂无实时风险数据。' : 'No live risk data.'}</div>
  }
  ...
  const exposureTag: Verdict =
    m.totalNotional === 0
      ? { text: t('flatVerdict', language), tone: 'muted' }
      : biasSkew > 15
        ? { text: t('longLean', language), tone: 'up' }
        : biasSkew < -15
          ? { text: t('shortLean', language), tone: 'dn' }
          : { text: t('balanced', language), tone: 'ink' }

  const levTag: Verdict =
    m.configMax === 0 || m.avgLev === 0
      ? { text: '—', tone: 'muted' }
      : m.levUse > 80
        ? { text: t('risky', language), tone: 'dn' }
        : m.levUse >= 50
          ? { text: t('high', language), tone: 'amber' }
          : { text: t('safe', language), tone: 'up' }

  const marginTag: Verdict =
    m.marginPct > 80
      ? { text: t('risky', language), tone: 'dn' }
      : m.marginPct >= 50
        ? { text: t('tight', language), tone: 'amber' }
        : { text: t('ample', language), tone: 'up' }

  const concTag: Verdict =
    m.totalNotional === 0
      ? { text: '—', tone: 'muted' }
      : m.concentration >= 35
        ? { text: t('concentrated', language), tone: 'amber' }
        : { text: t('spread', language), tone: 'up' }

  const ddTag: Verdict =
    m.drawdown <= 0
      ? { text: t('calm', language), tone: 'up' }
      : m.drawdown >= 20
        ? { text: t('deep', language), tone: 'dn' }
        : { text: t('caution', language), tone: 'amber' }

  const countTag: Verdict =
    m.maxPositions === 0
      ? { text: `${m.count}`, tone: 'muted' }
      : m.count >= m.maxPositions
        ? { text: t('full', language), tone: 'amber' }
        : { text: t('room', language), tone: 'up' }

  return (
    <div style={{ fontFamily: 'var(--tm-mono)' }}>
      {/* header */}
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 1 }}>
        <span className="tm-px" style={{ fontSize: 11 }}>{t('riskRadar', language)}</span>
        <span
          className="tm-sc"
          style={{ marginLeft: 'auto', color: m.totalNotional > 0 ? 'var(--tm-up)' : 'var(--tm-muted)' }}
        >
          {m.totalNotional > 0 ? `● ${t('live', language)}` : `○ ${t('flat', language)}`}
        </span>
      </div>
      <div className="tm-sc" style={{ fontSize: 9, marginBottom: 8 }}>
        {t('riskRadarDesc', language)}
      </div>
...
function Label({ zh, en }: { zh: string; en: string }) {
  ...
  return (
    <span style={{ display: 'inline-flex', flexDirection: 'column', lineHeight: 1.2 }}>
      <span style={{ fontSize: 11, color: 'var(--tm-ink)', fontWeight: 600 }}>{language === 'zh' ? zh : en}</span>
      {language !== 'zh' && <span className="tm-sc" style={{ fontSize: 8, letterSpacing: '0.12em' }}>{en}</span>}
    </span>
  )
}
