    ...
    <div className="fxos-terminal" style={{ minHeight: '100vh', padding: 0 }}>
      ...
      <button
        ...
        style={{
          ...
          background: on
            ? 'var(--tm-up)'
            : 'color-mix(in srgb, var(--tm-ink) 14%, transparent)',
        ...
        createPortal(
          <span
            ...
            style={{
              ...
              marginLeft: 16,
              paddingLeft: 16,
              borderLeft: '1px solid var(--tm-rule)',
              fontSize: 11,
            }}
          >
      ...
      >
        ...
        >
          <span style={{ fontWeight: 500 }}>
            {selectedTrader?.trader_name ?? 'FXOS'}
          </span>
          <span>
            <span className="tm-sc">{t('model', language)} </span>
            {(() => {
          ...
          <span>
            <span className="tm-sc">{t('strategy', language)} </span>
            {config?.strategy_name || selectedTrader?.strategy_name || '—'}
          </span>
          <span>
            <span className="tm-sc">{t('lev', language)} </span>
            {config?.btc_eth_leverage ?? '—'}× /{' '}
            {config?.altcoin_leverage ?? '—'}×
          </span>
          <span>
            <span className="tm-sc">{t('scanInterval', language)} </span>
            {scanMin}
            {language === 'zh' ? '分钟' : 'm'}
          </span>
          <span>
            <span className="tm-sc">{t('universe', language)} </span>
            {candidateCoins.length}
          </span>
          <span>
            <span className="tm-sc">{t('positionsLabel', language)} </span>
            {positions?.length ?? 0}
          </span>
          <span style={{ marginLeft: 'auto' }}>
            <span className="tm-sc">{t('nextCycle', language)} </span>
            {countdown === 'due now'
              ? language === 'zh'
                ? '即将开始'
                : 'due now'
          ...
          {[
            ...
            },
            {
              l:
                t('totalPnL', language) +
                ' · ' +
                (language === 'zh' ? '含未实现' : 'incl. unrealized'),
            ...
            {
              l: language === 'zh' ? '盈亏比' : 'Profit factor',
              v: fullStats != null ? fullStats.profit_factor.toFixed(2) : '—',
              c: 'var(--tm-ink)',
            },
            {
              l: language === 'zh' ? '最大回撤' : 'Max drawdown',
              v:
                fullStats != null
          ...
          <>
            ...
            >
              ...
              <span className="tm-sc tm-up">
                {language === 'zh' ? '盈利' : 'win'} {fullStats.win_trades} (
                {fullStats.win_rate.toFixed(1)}%)
              </span>
              <span className="tm-sc tm-dn">
                {language === 'zh' ? '亏损' : 'loss'} {fullStats.loss_trades}
              </span>
              ...
              <span className="tm-sc">
                {t('sharpeTrade', language)}{' '}
                <b style={{ color: 'var(--tm-ink)' }}>
                  {fullStats.sharpe_ratio.toFixed(2)}
                </b>
              </span>
              <span className="tm-sc">
                {t('avgWinLoss', language)}{' '}
                <b style={{ color: 'var(--tm-ink)' }}>
        ...
        <div style={sc}>
          ...
          >
            <span className="tm-px" style={{ fontSize: 12 }}>
              {t('orchestrationTopology', language)}
            </span>
            <span className="tm-sc">
              {t('orchestrationTopologyDesc', language)}
            </span>
          </div>
        ...
        >
          <div style={{ ...sc, borderRight: cellBorder }}>
            <ExecutionLog
              decisions={decisions}
              height={432}
              onCycleClick={setDetailDecision}
            />
          </div>
          <div style={{ ...sc, borderRight: cellBorder }}>
            <RiskRadar
          ...
          <div style={sc}>
            ...
            >
              ...
              <span className="tm-sc">
                {t('currentPositions', language)} · live
              </span>
              <span className="tm-sc" style={{ marginLeft: 'auto' }}>
                {positions?.length ?? 0} {language === 'zh' ? '个持仓' : 'open'}
              </span>
            </div>
            ...
            >
              ...
              <span className="tm-sc">
                Recent closes · symbol/side/time/pnl
              </span>
            </div>
        ...
        >
          <div style={{ ...sc, borderRight: cellBorder }}>
            ...
            >
              <span className="tm-px" style={{ fontSize: 12 }}>
                {t('marketNetInflow', language)}
              </span>
              <span className="tm-sc">
                {t('marketNetInflow', language)} · {flow?.data?.window || '1h'}{' '}
                · Vergex
              </span>
              <span className="tm-sc" style={{ marginLeft: 'auto' }}>
                {flowItems.length} {language === 'zh' ? '个市场' : 'markets'}
              </span>
            </div>
          ...
          <div style={{ ...sc, borderRight: cellBorder }}>
            ...
            >
              ...
              <span className="tm-sc">
                {t('bySymbolHistory', language)} · {t('tradesWinPnl', language)}
              </span>
            </div>
                ...
                <div key={s.symbol} style={{ marginBottom: 7 }}>
                  ...
                  >
                    ...
                    <span className="tm-sc" style={{ marginLeft: 8 }}>
                      {s.total_trades} {t('trades', language)} ·{' '}
                      {s.win_rate.toFixed(0)}% {t('win', language)}
                    </span>
                    <span
