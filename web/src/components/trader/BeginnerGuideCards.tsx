...
export function BeginnerGuideCards({
  ...
  return (
    <section className="space-y-4 rounded-[28px] border border-fxos-gold/20 bg-fxos-bg-lighter p-5">
      <div className="flex items-center justify-between gap-4">
        <div>
          <div className="text-xs font-semibold uppercase tracking-[0.3em] text-fxos-gold/80">
            {isZh ? 'Quickstart' : 'Quickstart'}
          </div>
          <h2 className="mt-1 text-xl font-bold text-fxos-text">
            {isZh
              ? 'Follow these 4 steps to get started fast'
              : 'Follow these 4 steps to get started fast'}
          </h2>
        </div>
        {/* <div className="rounded-full border border-fxos-gold/20 bg-fxos-bg-deeper px-3 py-1 text-xs text-fxos-text-muted">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {cards.map((card) => {
          ...
          return (
            <div
              key={card.key}
              className="rounded-[22px] border border-fxos-gold/20 bg-fxos-bg-deeper p-4"
            >
              <div className="flex items-center justify-between gap-3">
                <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-fxos-gold/10 text-fxos-gold">
                  <Icon className="h-5 w-5" />
                </div>
                <span
                  className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.22em] ${
                    card.ready
                      ? 'bg-fxos-success/15 text-fxos-success'
              ...
              <h3 className="mt-4 text-base font-semibold text-fxos-text">
                {card.title}
              </h3>
              ...
              <button
                ...
                className={`mt-5 w-full rounded-2xl px-4 py-3 text-sm font-semibold transition ${
                  card.disabled
                    ? 'cursor-not-allowed bg-fxos-bg-deeper text-fxos-text-muted'
                    : 'bg-fxos-gold text-white hover:bg-fxos-gold/90'
                }`}
              >
                {card.actionLabel}
              </button>
