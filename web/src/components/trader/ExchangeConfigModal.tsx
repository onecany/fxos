...
function ExchangeCard({
  ...
  return (
    <button
      ...
      style={{
        background: selected ? 'var(--fxos-gold-dim)' : 'var(--panel-bg)',
        border: selected ? '2px solid var(--fxos-gold)' : '2px solid var(--panel-border)',
      }}
    >
      ...
      <span
        ...
        style={{
          background: template.type === 'cex' ? 'var(--fxos-gold-dim)' : 'var(--fxos-gold-dim)',
          color: template.type === 'cex' ? 'var(--fxos-gold)' : 'var(--fxos-gold)',
        }}
      >
...
export function ExchangeConfigModal({
  ...
  const exchangeRegistrationLinks: Record<string, { url: string; hasReferral?: boolean }> = {
    binance: { url: 'https://www.binance.com/join?ref=FXOSENG', hasReferral: true },
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 overflow-y-auto backdrop-blur-sm">
      <div
        ...
        style={{ background: 'var(--panel-bg)', maxHeight: 'calc(100vh - 4rem)' }}
      >
        {/* Header */}
        <div className="flex items-center justify-between p-6 pb-2">
          <div className="flex items-center gap-3">
            {currentStep > 0 && !editingExchangeId && (
              <button type="button" onClick={handleBack} className="p-2 rounded-lg hover:bg-fxos-gold-dim transition-colors">
          ...
          <div className="flex items-center gap-2">
            {currentExchangeType === 'binance' && currentStep === 1 && (
              <button
                ...
                style={{ background: 'var(--fxos-gold-dim)', color: 'var(--fxos-gold)' }}
              >
            ...
            {editingExchangeId && (
              <button
                ...
                className="p-2 rounded-lg hover:bg-fxos-danger/20 transition-colors"
                style={{ color: 'var(--fxos-danger)' }}
              >
                <Trash2 className="w-4 h-4" />
              </button>
            )}
            <button type="button" onClick={onClose} className="p-2 rounded-lg hover:bg-fxos-gold-dim transition-colors" style={{ color: 'var(--text-secondary)' }}>
              ✕
            </button>
          </div>
        </div>
        ...
        <div className="px-6 pb-6 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 16rem)' }}>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="text-xs font-medium uppercase tracking-wide" style={{ color: 'var(--fxos-gold)' }}>
          ...
          {(currentStep === 1 || editingExchangeId) && selectedTemplate && (
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="p-4 rounded-xl flex items-center gap-4" style={{ background: 'var(--panel-bg)', border: '1px solid var(--panel-border)' }}>
                ...
                <a
                  ...
                  style={{ background: 'var(--fxos-gold-dim)', border: '1px solid var(--fxos-gold-glow)' }}
                >
                  <UserPlus className="w-4 h-4" style={{ color: 'var(--fxos-gold)' }} />
                  ...
                  {exchangeRegistrationLinks[currentExchangeType || '']?.hasReferral && (
                    <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: 'rgba(46, 139, 87, 0.2)', color: 'var(--fxos-success)' }}>
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2 text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>
                  <Key className="w-4 h-4" style={{ color: 'var(--fxos-gold)' }} />
                ...
                <>
                  {currentExchangeType === 'binance' && (
                    <div
                      ...
                      style={{ background: 'var(--fxos-gold-dim)', border: '1px solid var(--fxos-gold-dim)' }}
                      onClick={() => setShowBinanceGuide(!showBinanceGuide)}
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span style={{ color: 'var(--fxos-gold)' }}>ℹ️</span>
                      ...
                      {showBinanceGuide && (
                        <div className="mt-3 pt-3 text-sm" style={{ borderTop: '1px solid var(--fxos-gold-dim)', color: 'var(--text-primary)' }}>
                          <a
                  ...
                  {editingExchangeId && selectedExchange && (
                    <div
                      ...
                      style={{ background: 'rgba(46, 139, 87, 0.08)', border: '1px solid rgba(46, 139, 87, 0.2)', color: 'var(--fxos-success)' }}
                    >
                  ...
                  {currentExchangeType === 'binance' && (
                    <div className="p-4 rounded-xl" style={{ background: 'var(--fxos-gold-dim)', border: '1px solid var(--fxos-gold-dim)' }}>
                      <div className="text-sm font-semibold mb-2" style={{ color: 'var(--fxos-gold)' }}>
                      ...
                      {loadingIP ? (
                        <div className="text-xs" style={{ color: 'var(--text-secondary)' }}>{t('loadingServerIP', language)}</div>
                      ) : serverIP?.public_ip ? (
                        <div className="flex items-center gap-2 p-3 rounded-lg" style={{ background: 'var(--panel-bg)' }}>
                          <code className="flex-1 text-sm font-mono" style={{ color: 'var(--fxos-gold)' }}>{serverIP.public_ip}</code>
                          <button
                            type="button"
              ...
              {currentExchangeType === 'aster' && (
                <>
                  <div className="p-4 rounded-xl" style={{ background: 'var(--fxos-gold-dim)', border: '1px solid var(--fxos-gold-glow)' }}>
                    <div className="flex items-start gap-2">
                      <span style={{ fontSize: '16px' }}>🔐</span>
                      <div>
                        <div className="text-sm font-semibold mb-1" style={{ color: 'var(--fxos-gold)' }}>{t('asterApiProTitle', language)}</div>
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-2 text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>
                      ...
                      <Tooltip content={t('asterUserDesc', language)}>
                        <HelpCircle className="w-4 h-4 cursor-help" style={{ color: 'var(--fxos-gold)' }} />
                      </Tooltip>
                    </label>
                ...
                <div className="space-y-4">
                  <div className="p-4 rounded-xl" style={{ background: 'var(--fxos-gold-dim)', border: '1px solid var(--fxos-gold-glow)' }}>
                    <div className="flex items-start gap-2">
                      <span style={{ fontSize: '16px' }}>🔐</span>
                      <div>
                        <div className="text-sm font-semibold mb-1" style={{ color: 'var(--fxos-gold)' }}>
                ...
                <>
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-2 text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>
                      ...
                      <button type="button" onClick={() => setSecureInputTarget('lighter')} className="text-xs underline" style={{ color: 'var(--fxos-gold)' }}>{t('secureInputButton', language)}</button>
                    </label>
              ...
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={handleBack} className="flex-1 px-4 py-3 rounded-xl text-sm font-semibold transition-all hover:bg-fxos-gold-dim" style={{ background: 'var(--fxos-bg-lighter)', color: 'var(--text-secondary)' }}>
                ...
                {currentExchangeType !== 'hyperliquid' && (
                  <button
                    ...
                    style={{ background: 'var(--fxos-gold)', color: '#fff' }}
                  >
      ...
      {showGuide && (
        <div className="fixed inset-0 bg-black/75 flex items-center justify-center z-50 p-4" onClick={() => setShowGuide(false)}>
          <div className="rounded-2xl p-6 w-full max-w-4xl" style={{ background: 'var(--panel-bg)' }} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
                <BookOpen className="w-6 h-6" style={{ color: 'var(--fxos-gold)' }} />
                {t('binanceSetupGuide', language)}
              </h3>
              <button onClick={() => setShowGuide(false)} className="px-4 py-2 rounded-lg text-sm font-semibold" style={{ background: 'var(--fxos-bg-lighter)', color: 'var(--text-secondary)' }}>
