...
export function ModelConfigModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 overflow-y-auto backdrop-blur-sm">
      <div
        className="rounded-2xl w-full max-w-[52rem] relative my-8 shadow-2xl bg-fxos-bg-lighter"
        style={{
          ...
          <div className="flex items-center gap-3">
            {currentStep > 0 && !editingModelId && (
              <button
                type="button"
                onClick={handleBack}
                className="p-2 rounded-lg hover:bg-fxos-bg-deeper transition-colors"
              >
                <svg
                  className="w-5 h-5"
...
function Claw402ConfigForm({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await fetch(apiUrl('/wallet/validate'), {
          method: 'POST',
          ...
          <div className="flex gap-2">
            <input
              ...
              style={{
                ...
                border: keyError
                  ? '1px solid var(--fxos-danger)'
                  : walletAddress
                    ? '1px solid var(--fxos-success)'
                    : '1px solid var(--panel-border)',
            ...
            {!apiKey && !walletAddress && (
              <button
                ...
                onClick={async () => {
                  try {
                    const res = await fetch(apiUrl('/wallet/generate'), {
                      method: 'POST',
                    })
                    const data = await res.json()
                    if (data.private_key) {
...
function StandardProviderConfigForm({
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      >
        <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-fxos-bg-deeper border border-fxos-gold/20">
      ...
      {selectedModel.provider === 'kimi' && (
        ...
        >
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm" style={{ color: 'var(--fxos-danger)' }}>
