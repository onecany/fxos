...
import { FxosSelect } from '../ui/select'
...
> = {
  binance: {
    url: 'https://www.binance.com/join?ref=FXOSENG',
    hasReferral: true,
  },
...
export function TraderConfigModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm p-4 overflow-y-auto">
      <div
        className="bg-fxos-bg-lighter border border-fxos-gold/20 rounded-xl shadow-2xl max-w-2xl w-full my-8"
        style={{ maxHeight: 'calc(100vh - 4rem)' }}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-fxos-gold/20 bg-fxos-bg-lighter sticky top-0 z-10 rounded-t-xl">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-fxos-gold flex items-center justify-center text-white">
              {isEditMode ? (
            ...
            <div>
              <h2 className="text-xl font-bold text-fxos-text">
                {isEditMode
                  ? t('editTrader', language)
                  : t('createTrader', language)}
              </h2>
              <p className="text-sm text-fxos-text-muted mt-1">
                {isEditMode
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-lg text-fxos-text-muted hover:text-fxos-text hover:bg-fxos-bg-deeper transition-colors flex items-center justify-center"
          >
        ...
        >
          {/* Basic Info */}
          <div className="bg-fxos-bg border border-fxos-gold/20 rounded-lg p-5">
            <h3 className="text-lg font-semibold text-fxos-text mb-5 flex items-center gap-2">
              <span className="text-fxos-gold">1</span>{' '}
              {t('basicConfig', language)}
            </h3>
            <div className="space-y-4">
              <div>
                <label className="text-sm text-fxos-text block mb-2">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-fxos-bg-lighter border border-fxos-gold/20 rounded text-fxos-text focus:border-fxos-gold focus:outline-none"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <FxosSelect
                    ...
                    className="w-full px-3 py-2 bg-fxos-bg-lighter border border-fxos-gold/20 rounded text-fxos-text"
                    ...
                    (() => {
                      ...
                      return (
                        <a
                          ...
                          className="mt-2 inline-flex items-center gap-1.5 text-xs text-fxos-text-muted hover:text-fxos-gold transition-colors"
                        >
                          <UserPlus className="w-3.5 h-3.5" />
                          <span>{t('noExchangeAccount', language)}</span>
                          {regLink.hasReferral && (
                            <span className="px-1.5 py-0.5 bg-fxos-gold/10 text-fxos-gold rounded text-[10px]">
                              {t('discount', language)}
                            </span>
                          )}
          ...
          <div className="bg-fxos-bg border border-fxos-gold/20 rounded-lg p-5">
            <h3 className="text-lg font-semibold text-fxos-text mb-5 flex items-center gap-2">
              <span className="text-fxos-gold">2</span>{' '}
              {t('selectTradingStrategy', language)}
              <Sparkles className="w-4 h-4 text-fxos-gold" />
            </h3>
            <div className="space-y-4">
              <div>
                <label className="text-sm text-fxos-text block mb-2">
                ...
                />
                {strategies.length === 0 && (
                  <p className="text-xs text-fxos-text-muted mt-2">
              ...
              {/* Strategy Preview */}
              {selectedStrategy && (
                <div className="mt-3 p-4 bg-fxos-bg-lighter border border-fxos-gold/20 rounded-lg">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-fxos-gold text-sm font-medium">
                      {t('strategyDetails', language)}
                    </span>
                    {selectedStrategy.is_active && (
                      <span className="px-2 py-0.5 bg-fxos-success/20 text-fxos-success text-xs rounded">
                        {t('activating', language)}
                      </span>
                    )}
                  </div>
                  <p className="text-sm text-fxos-text-muted mb-2">
                  ...
                  selectedStrategy.config.grid_config ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-fxos-text-muted">
                      <div>
          ...
          <div className="bg-fxos-bg border border-fxos-gold/20 rounded-lg p-5">
            <h3 className="text-lg font-semibold text-fxos-text mb-5 flex items-center gap-2">
              <span className="text-fxos-gold">3</span>{' '}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <button
                      ...
                      className={`flex-1 px-3 py-2 rounded text-sm ${
                        formData.is_cross_margin
                          ...
                          : 'bg-fxos-bg-lighter text-fxos-text-muted border border-fxos-gold/20'
                      }`}
                    >
                ...
                <div>
                  ...
                  <input
                    ...
                    step="1"
                  />
                  <p className="text-xs text-fxos-text-muted mt-1">
              ...
              <div className="p-3 bg-fxos-bg-lighter border border-fxos-gold/20 rounded flex items-center gap-2">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="w-4 h-4 text-fxos-gold"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
        ...
        {/* Footer */}
        <div className="flex justify-end gap-3 p-6 border-t border-fxos-gold/20 bg-fxos-bg-lighter sticky bottom-0 z-10 rounded-b-xl">
          <button
            onClick={onClose}
            className="px-6 py-3 bg-fxos-bg-deeper text-fxos-text rounded-lg hover:bg-fxos-bg transition-all duration-200 border border-fxos-gold/20"
          >
            {t('cancel', language)}
          </button>
          {onSave && (
            <button
              ...
              className="px-8 py-3 bg-fxos-gold text-white rounded-lg hover:bg-fxos-gold/90 transition-all duration-200 disabled:bg-fxos-bg-deeper disabled:text-fxos-text-muted disabled:cursor-not-allowed font-medium shadow-lg"
            >
              {isSaving
                ? t('saving', language)
                : isEditMode
