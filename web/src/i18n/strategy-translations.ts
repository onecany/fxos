// FXOS i18n Consolidation - Translation Keys
...
export const coinSource = {
  ...
  fxososNote: { zh: '使用 FXOS API Key（在指标配置中设置）', en: 'Uses FXOS API Key (set in Indicators config)', es: 'Usa API Key de FXOS' },
...
export const indicator = {
  ...
  fxososTitle: { zh: 'FXOS 量化数据源', en: 'FXOS Data Provider', es: 'Proveedor FXOS' },
  fxososDesc: { zh: '专业加密货币量化数据服务', en: 'Professional crypto quant data service', es: 'Servicio crypto quant' },
  fxososFeatures: { zh: 'AI500 · OI排行 · 资金流向 · 涨跌榜', en: 'AI500 · OI Ranking · Fund Flow · Price Ranking', es: 'AI500 · OI · NetFlow · Ranking' },
  viewApiDocs: { zh: 'API 文档', en: 'API Docs', es: 'Docs API' },
  apiKey: { zh: 'API Key', en: 'API Key', es: 'API Key' },
  apiKeyPlaceholder: { zh: '输入 FXOS API Key', en: 'Enter FXOS API Key', es: 'Ingresar API Key' },
  ...
  fxososDataSources: { zh: 'FXOS 数据源', en: 'FXOS Data Sources', es: 'Fuentes FXOS' },
  configureApiKey: { zh: '请配置 API Key 以启用 FXOS 数据源', en: 'Please configure API Key to enable FXOS data sources', es: 'Configure API Key para habilitar FXOS' },
};
