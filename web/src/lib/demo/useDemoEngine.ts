...
function build(S: SimState): DemoDataset {
  ...
  const decisions: DecisionRecord[] = Array.from({ length: 4 }).map((_, k) => {
    ...
    const acts = S.positions.slice(0, 6).map((p) => ({
      ...
      reasoning:
        'Signal Lab confirms trend; cost/liq structure supports the level.',
  ...
  return {
    ...
    config: {
      ...
      ai_model: 'claw402',
      strategy_name: 'FXOS Claw402 Auto Strategy',
      btc_eth_leverage: 10,
      altcoin_leverage: 10,
    },
    flow,
...
export function useDemoEngine(active: boolean): DemoDataset | null {
  ...
  // react-hooks/refs: reading ref.current during render is deliberate here —
  // the tick() forceUpdate above guarantees a re-render after every interval
  // mutation, so the read is always fresh. A state rewrite would require
  // deep-copying the whole simulation tree every tick for zero benefit.
