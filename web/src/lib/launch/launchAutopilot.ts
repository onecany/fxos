...
export const AUTOPILOT_TRADER_NAME = 'FXOS Autopilot'
...
export async function launchAutopilot(
  ...
  try {
    ...
    // props/snapshots would create a duplicate "FXOS Autopilot" — paying the
    ...
    return {
      ...
      kind: 'error',
      message:
        err instanceof Error ? err.message : 'Failed to launch FXOS Autopilot',
    }
  }
}
...
export async function ensureClaw402Strategy(): Promise<string> {
  ...
  const created = await api.createStrategy({
    name: 'FXOS Claw402 Auto Strategy',
    description:
